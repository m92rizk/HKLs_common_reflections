...
void read_reflections(std::ifstream& file, std::vector<std::vector<std::string>>& table) {
    ...
    while (std::getline(file, line)) {
        ...
        if (in_table) {
            ...
            std::string k=line.substr(8,5);
            std::cout<<"K"<<k<<std::endl;
            std::string l=line.substr(15,5);
            std::cout<<"L"<<l<<std::endl;
            std::string I=line.substr(22,10);
            std::string sigma=line.substr(33,10);
            std::string sign=line.substr(43,1);


            std::cout<<line<<std::endl;
            //    45,     5,     1, 1.133E+00, 2.466E-01,0
            // hhhhh  kkkkk  lllll  iiiiiiiii  sssssssss s
            ...
            push_back_padded(new_row, sign, 2, 2);
            if (!printed_once) {
